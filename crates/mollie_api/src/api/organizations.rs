...
pub struct OrganizationsApi<'client> {
    api_client: &'client mut ApiClient<'client>,
}

impl<'client> OrganizationsApi<'client> {
    /// Create a new `OrganizationsApi`.
    pub fn new(api_client: &'client mut ApiClient<'client>) -> Self {
        Self { api_client }
    }

    /// [Get organization by id](https://docs.mollie.com/reference/v2/organizations-api/get-organization)
    pub async fn get_by_id(&mut self, id: &str) -> crate::Result<Organization> {
    ...
    pub async fn me(&mut self) -> crate::Result<Organization> {
    ...
    pub async fn get_partner(&mut self) -> crate::Result<Partner> {
